#ifndef ENDSCREENSTATE_H
#define ENDSCREENSTATE_H
#include "GameState.h"
...
{
public:
	EndScreenState(GameApp* app) : GameState(app)
	{
	...
	virtual bool update(ESContext* ctx, float deltaTime)
	{
		...
		batch->addText(fontTexture, text, vec2(-ctx->width / 35, ctx->height / 10), 0, vec2(6, 6));
		batch->addText(fontTexture, text2, vec2(-ctx->width / 14, ctx->height / 6 - ctx->height / 2), 0, vec2(3, 3));
